...
columna3=[]
count=0
colores = plt.cm.viridis(np.linspace(0, 1, len(valores_unicos)))
...
for valor in valores_unicos:
    ...
    for pos in posiciones:
        ...
        valor_cambiado=float(df.iloc[filas_que_cumplen[pos]-1, 16].replace(',', '.'))
    ...
    columna3.append(float((df.iloc[32, 16]).replace(',', '.')))
    print(columna2)
    print(columna3)
    factor_de_escala = 100
    columna3_escalada = [valor * factor_de_escala for valor in columna3]
